use crate::{
    ...
    native::tx::{TransferPub, TransferSec, PoolParams, PoolBN256, Note, Tx, tx_hash, note_hash, nullfifier, note_encrypt}
};
...
use num::bigint::{ToBigInt, ToBigUint, Sign};
use num::{One, Zero};
...
use fawkes_crypto::native::poseidon::{poseidon_with_salt, poseidon, MerkleProof};
...
impl <'p, P:PoolParams> ClientState<'p, P> {
    pub fn new(params:&'p P) -> Self {
        ...
        let sk_base64 = env::vars().find_map(|e| if e.0 == "PRIVATE_KEY" {Some(e.1)} else {None}).unwrap();
        let sk = Num::<<P::J as JubJubParams>::Fs>::try_from_slice(&decode(sk_base64).unwrap()).unwrap();
    ...
    fn get_cell(&self, pos:(usize,usize)) -> Num<P::F> {
        self.cell.get(&pos).unwrap().unwrap_or(self.default_cell_value[pos.0])
    }
    ...
    fn update_merkle_path(&self, mut pos:usize) {
        for i in 0..H::USIZE {
            pos>>=1;
            let h = poseidon(&[self.get_cell((i, pos*2)), self.get_cell((i, pos*2+1))], self.params.compress());
            self.set_cell((i+1, pos), h);
        }
    }

    fn get_merkle_proof(&self, pos:usize) -> MerkleProof<P::F, P::H> {
        let sibling = (0..P::H::USIZE).map(|i| self.get_cell((i, (pos>>i)^1))).collect();
    ...
    pub fn make_transaction_object<R:Rng>(&self, rng:&mut R, recv_addr: (Num<P::F>, Num<P::F>), amount: BigUint, delta: BigInt) -> Option<(TransferPub<P>, TransferSec<P>, Vec<u8>)> {
        assert!(P::OUT::USIZE>=2);
        
        let mut note = self.note.iter().map(|e| {
            let e = e.unwrap();
            (e.0, e.1, Into::<BigUint>::into(e.1.v))
        }).collect::<Vec<_>>();
        note.sort_by(|a,b| b.2.partial_cmp(&a.2).unwrap());

        let mut spending_amount: BigUint = note.iter().take(P::IN::USIZE).map(|e| e.2.clone()).sum();
        ...
        if spending_amount < amount {
            ...
            let tx = {
                let input = indexes.iter().map(|&i| note[i].1.clone() ).collect();
                
                let output = [sender_note, receiver_note].iter().cloned().chain((2..P::OUT::USIZE).map(|_| Note {
            ...
            let in_proof = indexes.iter().map(|&i| self.get_merkle_proof(note[i].0)).collect();
            let in_note_hash = tx.input.iter().map(|&e| note_hash(e, self.params)).collect::<Vec<_>>();
            let out_note_hash = tx.output.iter().map(|&e| note_hash(e, self.params)).collect::<SizedVec<_, P::OUT>>();

            let (eddsa_s, eddsa_r, eddsa_a) = {    
                let m = tx_hash(&in_note_hash, &out_note_hash.0, self.params);
                let (s, r) = eddsaposeidon_sign(self.sk, m, self.params.eddsa() , self.params.jubjub());
                (s.into_other(), r, self.pk)
            };

            let transfer_sec = TransferSec {
                tx, in_proof, eddsa_s, eddsa_r, eddsa_a
            };

            let transfer_pub = {
                let root = self.get_cell((P::H::USIZE, 0));
                let nullifier = in_note_hash.iter().map(|&e| nullfifier(e, self.pk, self.params)).collect();
                let out_hash = out_note_hash;
                let delta = prepare_delta(delta);
                
                
                TransferPub {
                    root, nullifier, out_hash, delta, memo
                }
            };
